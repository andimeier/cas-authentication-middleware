const CASAuthentication = require("cas-authentication-ajax");
...
var options = {
  ...
  session_targetUrl: "cas_target_url",
...
function casLogin(req, res, next) {
  ...
  if (options.is_dev_mode) {
    ...
    req.session[options.session_info] = options.dev_mode_info;
    req.session[options.session_name] = options.dev_mode_user;
...
function startSession(req, res, next) {
  ...
  console.log(
    "handling ticket [" +
      ticket +
      "], redirecting to service_url [" +
      options.service_url +
      "]"
  );
  cas.handleTicketAjax(ticket, options.service_url, function(
    err,
    username,
    userAttributes
  ) {
...
function startAppSession(req, username, userData) {
  ...
  // -------------
  req.session = req.session || {};
  req.session[options.session_name] = username;
  if (options.session_info) {
    // store any additional data from the CAS service
    req.session[options.session_info] = userData || {};
  }
...
function login(req, res) {
  req.session = req.session || {};
  req.session[options.session_name] = "alex12";
...
function isActiveSession(session) {
  ...
  return session && session[options.session_name];
}
