...
| onUnauthorizedUser | `(req: Express.Request, res: Express.Response, username: string, err: any) => void` | `null` | This function is called when the user was not auhtorized (the `checkUser` function rejeted). It is passed the [`Express.Request`](https://expressjs.com/en/4x/api.html#req) object to access the session, the [`Express.Response`](https://expressjs.com/en/4x/api.html#res) object to send a custom response, the username from the CAS server and the error. |
