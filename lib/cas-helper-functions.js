...
function startSession(req, res, next) {
  ...
  if (options.devMode) {
    startSessionTicketCallback(req, res, next, ticket)(null, options.devModeUser);
  } else {
    cas.handleTicketAjax(ticket, options.serviceUrl, startSessionTicketCallback(req, res, next, ticket));
  }
  logger.silly("finished handling ticket");
}

function startSessionTicketCallback(req, res, next, ticket) {
  return function(err, username, userAttributes) {
    if (err) {
      logger.verbose(
        `${options.devMode ? "[dev mode] " : ""}!!!!! OUCH error from handleTicketAjax: ${err.code} - ${err.message}`
      );
      logger.error({
        code: 500,
        message: err.message,
    ...
    logger.verbose(
      (options.devMode ? "[dev mode] " : "") + 
      "successfully validated ticket " + ticket
    );
    ...
    if (options.checkUser) {
      ...
      options
        ...
        .catch(err => {
          ...
          } else if(options.onUnauthorizedUser(req, res, username, err)) {
            next();
          }
        });
    } else {
