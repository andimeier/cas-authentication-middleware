...
function getServiceUrl(req) {
  ...
  if (options.backendBaseUrl) {
    options.serviceUrl = options.backendBaseUrl + `/${casRouterPrefix}/validate`;
    return options.serviceUrl;
  }
  throw new Error('ups, keine BackendBaseUrl angegeben - der Rest funktioniert vielleicht nicht???'); // TODO funktioniert ohne backendBaseUrl nicht hinter einem reverse proxy:
