...
function casLogin(req, res, next) {
  ...
  cas.setServiceUrl(getServiceUrl(req));
...
function startSession(req, res, next) {
  ...
  if (options.devMode) {
    startSessionTicketCallback(req, res, next, ticket)({ user: options.devModeUser });
  } else {
    cas.handleTicketAjax(ticket, options.serviceUrl)
      .then(
        startSessionTicketCallback(req, res, next, ticket),
        (err) => {
          logger.verbose(
 ...
 * @param {string[]} paths
 */
function getUrlPathToSelf(...paths) {
...
module.exports = {
  ...
  startSession,
  casLogin,
  logout: CASAuthentication.logout,
  returnTargetUrl,
  init,
  options,
  registerOptions,
